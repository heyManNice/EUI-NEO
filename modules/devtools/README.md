...
- 选中的元素会在页面上高亮：页面 Runtime 在渲染收尾时画出它的外框与内容框（描边 + 淡填充），变换与祖先裁剪都和元素本身一致，所以滚动容器、缩放和 clip 里的元素也能对齐；离开 Elements 标签或隐藏面板时标识自动撤掉。
- 鼠标在树上悬浮某一行时，会用另一种颜色预览那个元素（悬浮预览画在选中高亮之上，移出列表即撤掉），点一下才变成选中。
- 在页面上拾取元素（点选）尚未实现。
